...
function createCandidateFromRow(row: Record<string, unknown>, fallbackId: string, business: string): MatchCandidate {
  const title = pickString(row, ["keyword", "title", "subject", "sj"]) || fallbackId;
  const rawContent = pickString(row, ["contents", "content", "summary"]);
  const content = stripHtml(rawContent);

  const boardNo = pickString(row, ["boardno"]);
  const url = pickString(row, ["url", "link", "filepath"]);

  const rowKeywords = [
    pickString(row, ["business"]),
    pickString(row, ["keyword"]),
    business,
  ].filter(Boolean);
  const date = parseCandidateDate(row, title, content);

  return {
    id: boardNo || fallbackId,
    title,
    content,
...
function dedupeCandidates(candidates: MatchCandidate[]) {
  ...
  for (const candidate of candidates) {
    const hasStableId = Boolean(candidate.id) && !candidate.id.startsWith("case-");
...
serve(async (req) => {
  ...
  if (!hasSuccessfulResponse && lastFailure !== "UNKNOWN_ERROR") {
    return errorResponse(502, "UPSTREAM_ERROR", "KOSHA disaster API failed for all query variants.", lastFailure);
  }

  const candidates = dedupeCandidates(collectedRows.map((row, rowIndex) => createCandidateFromRow(row, `case-${rowIndex + 1}`, business)));
  if (candidates.length === 0) {
    return jsonResponse([], 200, { "x-risk-guard-source": "kosha-disaster-cases" });
  }
  ...
  return jsonResponse(mapped, 200, { "x-risk-guard-source": "kosha-disaster-cases" });
});
