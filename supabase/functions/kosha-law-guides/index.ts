...
serve(async (req) => {
  ...
  try {
    ...
    console.warn("[kosha-law-guides] DEPRECATED_ENDPOINT: legacy endpoint is active; use kosha-law-guides-assessment or kosha-law-guides-form.");
    const payload = await buildLawGuidesPayload(body, { mode: "assessment" });
    return jsonResponse(payload, 200, {
      "x-risk-guard-source": "kosha-law-guides",
      "x-risk-guard-deprecated": "true",
    });
  } catch (error) {
    console.error("[kosha-law-guides] Unhandled error", error);
