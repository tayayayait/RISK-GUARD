...
- API-only + evidence-only 경로(`kosha-law-evidence`)는 워커 보호를 위해 검색어를 최대 `10`개로 제한
- smartSearch row 수: 법령 카테고리(1~4) `numOfRows=60`, Guide/미디어 `numOfRows=20`
- smartSearch timeout: per-request `8000ms`, total budget `70000ms`
